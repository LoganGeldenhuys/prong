...
namespace Prong {
        ...
        static std::string getModelIdentifierAsString(ModelIdentifier m) {
            if (std::holds_alternative<ModelType>(m)) {
                switch (std::get<ModelType>(m)) {
                    case ModelType::GPT_4_32K: return "gpt-4-32k";
                    case ModelType::GPT_4: return "gpt-4";
                    case ModelType::GPT_4_TURBO_PREVIEW: return "gpt-4-turbo-preview";
                    case ModelType::GPT_3_5_TURBO: return "gpt-3.5-turbo";
    ...
    protected:
        virtual std::string getBaseUrl_() const override {
            return baseUrl_;
        }

        virtual std::string getPort_() const override {
            return port_;
        }

        virtual std::string getBasePath_() const override {
            return basePath_;
        }
		...
		virtual int getVersion_() const override {
			return version_;
        }

		std::string getRequestBody_(const std::vector<Message>& messages) const override{
			...
			boost::property_tree::write_json(buf, pt, false); // Set false for no pretty printing
			return buf.str();
		}
		...
		// Function to parse the response from the OpenAI API and construct a Message object
		virtual Message parseResponse_(const boost::property_tree::ptree& pt) const override{
		...
		virtual std::string getRequestBody_Stream(const std::vector<Message>& messages) const override {
    ...
    public:
        explicit OpenAIChatModel(ModelIdentifier modelIdentifier) : modelIdentifier_(getModelIdentifierAsString(modelIdentifier)) {
