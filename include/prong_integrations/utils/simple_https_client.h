...
// This is adapted from a Boost example HTTPS client by Vinnie Falco, see copyright below
//
//-----------------------------------------------------------------------------
...
namespace Prong {
	...
	void load_system_certificates(ssl::context& ctx) {
		...
		if (!fs::exists(cert_path) || !fs::is_directory(cert_path)) {
			throw std::runtime_error("Certificate directory not found or is not a directory.");
		}
		...
		for (const fs::directory_entry& entry : fs::directory_iterator(cert_path)) {
			if (entry.is_regular_file()) {
				...
				if (!cert_file) {
					std::cerr << "Failed to open certificate file: " << cert_file_path << std::endl;
				...
				try {
					...
					//std::cerr << "Failed to load certificate: " << cert_file_path << "; Error: " << e.what() << std::endl;
				}
			}
		}
	}
	...
	std::string httpsRequest(
		...
		const int version = 11) {
			try {
				...
				if(! SSL_set_tlsext_host_name(stream.native_handle(),static_cast<void*>(&my_host[0])))
				{
				...
				//if(ec)
				//throw beast::system_error{ec}; // I get cryptography but someone needs to tell OpenAI
