...
// This is a simple http client based on the sample http client provided at
// libs/beast/example/http/client/sync/http_client_sync.cpp in the Boost library
// v1.84
//
...
std::future<std::string> httpRequest(
  ...
  return std::async(
      std::launch::async,
      [host, port, target, verb, payload, headers, version]() -> std::string {
        try {
          net::io_context ioc;
          ...
          if (!payload.empty() &&
            ...
            req.body() = payload;
            req.prepare_payload();  // this sets the content-length and
          ...
          std::cout << "Request Method: " << req.method_string().to_string()
                    << std::endl;
          std::cout << "Request Target: " << req.target().to_string()
                    << std::endl;
          std::cout << "Request Version: "
                    << "HTTP/" << req.version() / 10 << "."
                    << req.version() % 10 << std::endl;
...
std::future<std::string> httpStreamRequest(
  ...
  return std::async(
      std::launch::async,
      [&os, host, port, target, verb, payload, headers,
       version]() -> std::string {
        try {
          net::io_context ioc;
          tcp::resolver resolver(ioc);
          ...
          std::function<void(std::uint64_t, boost::beast::string_view,
                             boost::system::error_code&)>
              on_chunk_header =
                  [&os, &touched](std::uint64_t size,
                                  boost::beast::string_view extensions,
                                  boost::system::error_code& ec) {
                    touched = true;
                    if (ec) {
                      std::cout << "Error during chunk header parsing: "
                                << ec.message() << std::flush;
                    } else {
          ...
          std::function<std::size_t(std::uint64_t, boost::beast::string_view,
                                    boost::system::error_code)>
              on_chunk_body =
                  [&os, &touched](std::uint64_t remain,
                                  boost::beast::string_view body,
                                  boost::system::error_code ec) -> std::size_t {
            touched = true;
            if (ec) {
              std::cout << "Error during chunk body parsing: " << ec.message()
                        << std::flush;
              return 0;  // Indicate that no octets were consumed due to the
                         // error
            } else {
              std::size_t consumed = body.size();  // Assume we can process the
                                                   // entire chunk at once
              std::cout << "Chunk body: remain=" << remain << ", body=\""
                        << body << "\"" << std::flush;
...
boost::property_tree::ptree httpRequestJson(
  ...
  try {
    ...
    auto future_response =
        httpRequest(host, port, target, verb, body, headers, version);
    std::string response =
        future_response.get();  // Get the response string from the future
...
std::future<boost::property_tree::ptree> httpRequestJsonStream(
  ...
  return std::async(
      std::launch::async,
      [&os, host, port, target, verb, body, headers,
       version]() -> boost::property_tree::ptree {
        try {
          // Reuse the http_get function to perform the HTTP GET request
          auto future_response = httpStreamRequest(os, host, port, target, verb,
