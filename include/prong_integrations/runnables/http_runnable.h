...
class HTTPRunnable : public Runnable<Input, Output> {
 protected:
  ...
  virtual Output parseResponse_(
      const boost::property_tree::ptree& pt) const = 0;

  // calls non-streaming methods by default, override if streaming behavior
  // should be different
  virtual std::string getBaseUrl_Stream() const { return getBaseUrl_(); }
  virtual std::string getPort_Stream() const { return getPort_(); }
  virtual std::string getBasePath_Stream() const { return getBasePath_(); }
 ...
 public:
  ...
  virtual Output operator()(const Input& input) const override {
    ...
    if (getPort_() == "443") {
      responsePTree = httpsRequestJson(
          getBaseUrl_(), getPort_(), getBasePath_(), getHTTPVerb_(),
          getRequestBody_(input), getHeaders_(), getVersion_());
    } else {
      responsePTree = httpRequestJson(getBaseUrl_(), getPort_(), getBasePath_(),
                                      getHTTPVerb_(), getRequestBody_(input),
                                      getHeaders_(), getVersion_());
    }
    // Parse the response and construct a Output object with the assistant's
    // reply
    try {
  ...
  virtual Output operator()(std::ostream& os,
    ...
    // Use http_request_json to make an asynchronous request to the OpenAI API
    auto futureResponse = httpRequestJsonStream(
        os, getBaseUrl_Stream(), getPort_Stream(), getBasePath_Stream(),
        getHTTPVerb_Stream(), getRequestBody_Stream(input),
        getHeaders_Stream());
