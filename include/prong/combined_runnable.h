...
class CombinedRunnable: public Runnable<Input, Output> {
	...
	public:
		CombinedRunnable(Runnable<Input, Intermediate>& left, Runnable<Intermediate, Output>& right) : left_(&left), right_(&right) {}
		...
		CombinedRunnable<Input, Output, NewOutput> operator|(Runnable<Output, NewOutput>& newRight) {
