...
namespace Prong {	
	...
	// A special type of prompt that returns a Message when passed a substitution (not a std::string like Prong::Prompt)
	...
	struct MessagePrompt{
		...
		template <class... Args>
		std::vector<Message> operator()(const std::vector<std::tuple<Args...>>& inputs) const {
		...
		PromptRunnable<MessagePrompt<messageRole, template_>, Message, NewOutput> operator|(Runnable<Message, NewOutput>& newRight) {
			return PromptRunnable<Prompt<template_s>, Message, NewOutput>(*this, newRight);
		}
	};
}
